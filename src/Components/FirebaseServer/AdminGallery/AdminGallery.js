...
const AdminGallery = () => {
    ...
    useEffect(() => {
        const fetchImages = async () => {
          db.collection('galleryimages').onSnapshot(snapshot => {
            setImageUrl(snapshot.docs.map((doc) => ({
              id: doc.id,
        ...
        const fetchVideo = async () => {
          db.collection('galleryvideos').onSnapshot(snapshot => {
            setVideoUrl(snapshot.docs.map((doc) => ({
              id: doc.id,
              url: doc.data().photoUrl,
              fileName: doc.data().pathName
            })));
    ...
    return (
        <>
            <div style={{position:'absolute',}}>{loading === true ? <Loading/>: ''}</div>
            <div className={styles.container}>
                <div className={styles.image__container}>
                    ...
                    <button className={styles.hom__sub} name='image'  disabled={!photoUrl || !fileName} onClick={submitResult}>
                        Submit
                    </button>
                </div>
                <div className={styles.image__container}>
                    <div>
                        ...
                        <input type='file' name='video' onChange={handleChange} ref={fileInputRef}></input>
                        <button name='video' onClick={handleUpload} disabled={!file}>Upload to Cloud</button>
                    </div>
                    <div><video src={photoUrl} alt="" style={{ width: "10%" }} /></div>
                    <button className={styles.hom__sub} name='video'disabled={!photoUrl || !fileName} onClick={submitResult}>
                        Submit
                    </button>
                </div>
                <table>
                    ...
                    {imageUrl.map((img,i) => (
                        <tr key={i}>
                            ...
                            <td>{img.fileName}</td>
                            <td>
                                <button onClick={() => handleDelete(img.id, img.fileName, 'image')}>Delete</button>
                            </td>
                        </tr>
                    ))}
                    </tbody>
